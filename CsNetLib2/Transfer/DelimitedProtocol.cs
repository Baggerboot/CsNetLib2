...
namespace CsNetLib2
{
	public class DelimitedProtocol : TransferProtocol
	{
		...
		public override List<DataContainer> ProcessData(byte[] buffer, int read, long clientId)
		{
			...
			for (int i = beginIndex; i < read; i++) { // Iterate over buffer
				if (buffer[i] == Delimiter[0]) { // We've found a delimiter
					...
					if (validDelimiter) {
						var data = ProcessMessage(buffer, beginIndex, i - Delimiter.Length, clientId); // Process a message from the begin index to the current position
						containers.Add(data);
		...
		private DataContainer ProcessMessage(byte[] buffer, int begin, int end, long clientId)
		{
